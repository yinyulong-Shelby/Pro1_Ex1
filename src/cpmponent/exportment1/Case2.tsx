...
const {Option} = Select
function Case2 (props: Case2Props, state: Case2State){
    ...
    function Scatter(){
        let height_l = (theme.height - 34) / 5 - 20
        let margin = {top: 10, left: 40, right: 10, bottom: height_l / 2}
        let width = theme.width - 4 - 17
        for(let i = 0; i < Id_list.length; i++){
            let data: {
                date: Date,
    ...
    let title = ["title_" + dataName].map((v) => {
        return(
            <div key={judge + v} style={{float: "left", width: theme.width, height: 30, fontSize: "14px", fontWeight: 600, lineHeight: '30px', color:'green'}}>
                <div style={{float: "left", width: 200, height: 30}}>&nbsp;{dataName === "NowData.csv" ? "当前算法的MAE数据分布" : "之前算法的MAE数据分布"}</div>
    ...
    if(dataset.length === 0){
        return(
            <div className="Case2" style={{position: 'absolute', ...theme}}>
                {title}
            </div>
        )
    } else {
        ...
        let line = dataset.map((v, index) => {
            return(
                <div key={'line' + judge + index} style={{float: 'left', width: theme.width - 4 - 17, height: each_height, borderTop: '2px solid rgb(150, 150, 150)', background: index % 2 === 0 ? "rgb(180, 180, 180)" : "white"}}>
        ...
        let circle = dataset.map((v, index) => {
            return(
                <div key={'circle' + judge + index} style={{float: 'left', width: theme.width - 4 - 17, height: each_height, borderTop: '2px solid rgb(150, 150, 150)', background: index % 2 === 0 ? "rgb(180, 180, 180)" : "white"}}>
                <div style={{float: 'left', marginLeft: '5px', width: 230, height: 20, lineHeight: '20px'}}>{Id_list[index]}层</div>
        ...
        return(
            <div className="Case2" style={{position:"absolute", ...theme}}>
                {title}
                {/* 折线图 */}
                <div style={{float: 'left', width: theme.width - 4, height: theme.height - 34, overflow: 'auto', display:"inline"}}>
                    {line}
                </div>
                {/* 散点图 */}
                <div style={{float: 'left', width: theme.width - 4, height: theme.height - 34, overflow: 'auto', display:"none"}}>
                    {circle}
                </div>
            </div>
        )
